...
The deliverable is no longer the end state. At the end of Phase A, the deliverable composer **writes the commitments** (pillars / KPIs / standards) into `st_commitments` as the starting taxonomy. Every stakeholder interview transcript, workshop decision, photo, and supporting document has already been auto-chunked into `knowledge_chunks` (scoped to the engagement) during the Phase A stages. Nera already knows the organisation's context in depth. The handover into Phase B is a status transition, not a content transfer — the knowledge is already in place.
...
**Commitments** (§6). The top-level taxonomy — what the organisation has agreed to pursue or maintain. Fixed at delivery or onboarding. Editable under a justification flow. The vocabulary is configurable per deployment (*pillar*, *KPI*, *standard*, *commitment*, *priority*, whatever the customer calls them).
...
- `type` — `pillar` / `initiative` / `dimension` (conceptual levels; vocabulary is customer-facing strings in `st_ai_config.vocabulary_map`)
...
**Dimensions** are not hierarchical — they're a separate commitment type with no parent, used as cross-cutting tags. They sit alongside the pillar/initiative hierarchy. See §8 for how Nera uses them.

### Count guidance

- **Recommended**: 3–5 pillars. Three is optimal. Five is doable.
- **Soft warning**: 6 pillars. Nera flags but does not block.
- **Hard cap**: 7 pillars. Above 7, the admin cannot create a new pillar without **archiving an existing one**. This is the "replacement rule" — it's not a total number, it's a live number.
...
The strictness of the justification flow when a new pillar is created is **itself a per-engagement configuration setting**, not a hardcoded product rule. Three levels:
...
When a document is being filed and the admin selects an existing pillar/initiative but notes "this slightly extends the scope to include X", the tool captures a structured `st_scope_extension` entry:
...
A subtle but important feature. When the admin creates a new pillar or initiative, Nera does **not** immediately check for duplication against existing ones — that's too aggressive and prevents legitimate nuance from emerging. Instead, Nera watches the new commitment for the first 30–60 days after creation.

If the evidence flowing into the new commitment looks semantically very similar to the evidence flowing into an existing commitment, Nera raises a merge suggestion: *"The content you've been filing under the new pillar 'Digital transformation readiness' over the last six weeks looks ~84% semantically similar to content filed under the existing pillar 'Operational modernisation'. Consider whether these should be merged. If you meant them to be distinct, I'll stop asking — but you might want to sharpen the boundary between them in both pillar descriptions."*

The admin can accept the merge (Nera handles the migration, updates the change log, marks the merged pillar with `merged_into_id`), reject the merge (Nera stops asking for that pair), or defer (Nera re-asks in 30 days). This is the mechanism that prevents taxonomy bloat from well-meaning but redundant additions.
...
## 8. Cross-cutting insights (Dimensions)

Dimensions are the non-hierarchical layer of the taxonomy. They emerged from the RWAV deliverable where "pilot communities", "financial strategy", and "implementation timeline" weren't pillars but were cross-cutting views that applied across all pillars. They remain first-class in the model because they are where the most valuable insights come from during Phase A workshops and survey analysis.

A dimension is just a commitment with `type='dimension'` and `parent_id=NULL`. Documents and evidence can be tagged with dimensions in addition to (not instead of) pillars/initiatives. Dimensions carry their own descriptions and success signals but no hierarchical children.

### Why dimensions belong in Nera's insights

The cross-referencing value comes from Nera looking at chunks through multiple axes at once. "Tell me everything in the corpus that touches the *financial strategy* dimension AND is tagged against the *rural health coalition* pillar" is a qualitatively different query than single-axis retrieval. It surfaces tensions, overlaps, and opportunities that no hierarchical structure alone can reveal. Nera's drift-watch and brief modes should both query across dimensions by default.

### Per-engagement governance

The taxonomy — including dimensions — is **per-engagement**. Every engagement defines its own commitments. `knowledge_chunks` must support per-engagement filtering and per-engagement commitment linking so that cross-cutting insights are scoped correctly. The `engagement_id` column on `knowledge_chunks` (mentioned in §5) is the enforcement mechanism. A strategic-tool query against `knowledge_chunks` is always predicated on `engagement_id = X` plus the commitment filter.

Dimensions are also configurable in the admin panel. Admins can add, rename, or archive them without triggering the pillar-count cap. Dimensions are lightweight metadata and Nera uses them aggressively; the customer should be able to evolve them freely.

## 9. Three input streams

### Documents

The primary stream. Drag-and-drop upload in the admin panel. Supports PDF, Word, Markdown, plain text, and images (which route through OCR). Each upload asks the admin to select a primary commitment and optionally additional commitment and dimension tags. A scope-extension note can be added inline. The file is stored in `st-documents` bucket, the chunking pipeline runs, chunks land in `knowledge_chunks` with appropriate metadata, and Nera generates a one-sentence summary for the document list view.
...
- **Vocabulary map** — `{ "commitment_top_singular": "pillar", "commitment_top_plural": "pillars", "commitment_sub_singular": "initiative", "commitment_sub_plural": "initiatives", "dimension_singular": "dimension", "commitment_add_verb": "introduce", "commitment_archive_verb": "retire", "evidence_singular": "document", ... }` — **every user-facing heading and placeholder in the UI renders through this map**
...
- **Default pillar count limits** — recommended range, soft warning, hard cap
- **Default justification strictness**

### Profiles we'll ship

- `strategic-planning` — the original use case. Vocabulary: pillars, initiatives, dimensions, board papers, drift, strategic plan. Default strictness: medium. Pillar range 3–5, hard cap 7.
- `grant-reporting` — for PHN contracts, state health agreements, federal grants. Vocabulary: KPIs, evidence items, funder reports, variations. Default strictness: hard (funders hate scope creep). Pillar range pre-set by the funder; hard cap unusable.
...
- `st_engagements` (with status enum `draft|active|delivered|living|archived`, type enum `strategic_planning|grant_reporting|governance|accreditation`, `profile_key`, `taxonomy_strictness`, `pillar_count_warning`, `pillar_count_hard_cap`)
...
- `st_commitments` (self-referencing tree, pillars/initiatives/dimensions)
...
- **Multi-tenant SaaS with billing**. One deployment per customer. Commercial extraction is a separate future project.
...
- Phase A delivers the commitments (pillars and dimensions) directly into `st_commitments` — the deliverable composer *is* the taxonomy writer.
- Phase A chunks all interview transcripts, workshop outputs, and supporting documents directly into the living-phase corpus.
- Phase A ends with a status transition from `delivered` to `living` and a handover flow that assigns the `admin` role to a named client user, with Carla retaining a `facilitator` role that gives her read access and the ability to re-open the engagement for refresh cycles.
- Phase A surveys conducted during workshops (or the community pulse survey RWAV ran) are ingested via the survey pipeline and become part of the living corpus.
- The deliverable document itself is chunked and linked to the commitments it describes, so Nera can always cite the founding document when explaining a commitment.

The handover is not a content transfer. It's a role flip. The content has been accumulating from day one of the engagement.
...
- **Cross-cutting dimensions**: first-class in the data model, Nera uses them for cross-referencing insights. ✓
- **Pillar count**: 3–5 recommended, 7 hard cap, replacement rule above the cap. ✓
- **Supabase strategy**: shared project with `st_*` prefix discipline, extraction plan maintained. ✓

### Still open

- **Preferred vocabulary defaults for the `strategic-planning` profile**. I've used "pillar / initiative / dimension" as placeholders. Carla's own consulting practice may have preferred terms (priorities, themes, streams, outcomes, drivers). What do you actually use?
- **The "dimension" concept's name**. "Dimension" is dry. Alternatives: "lens", "view", "cross-cut", "facet", "thread". Pick the one you'd say in a client meeting without wincing.
- **Default pulse-check cadence**. Weekly? Fortnightly? Monthly? Six-weekly? Probably six-weekly is the sweet spot for small boards — frequent enough to catch drift, infrequent enough not to annoy people.
- **Default drift-watch report delivery**. In-app only? Email? Slack? Notion page? Probably in-app plus email, with Slack/Notion as v1.5.
- **Handover role semantics**. When an engagement flips to `living`, does Carla retain full admin access, or does she drop to a read-only `facilitator` role with the ability to request elevation? The trust model affects how consultants think about selling this.
- **Healthcare PHI stance**. Confirm: "no patient-identifiable information ever lands in this tool" is the design constraint. If yes, simpler. If no, we have a data-classification and audit-log conversation to have first.
- **Subscription pricing model**. Not a schema question but worth resolving before we invest more. My earlier recommendation: no billing infrastructure in v1, invoice manually until 5+ customers are in `living` mode.
- **First real engagement**. Which customer (real or fictional) is phase 1 schema work targeting? The de-identified RWAV demo is fine as a stress test but the real first customer matters for prioritisation of which surfaces to build first.
